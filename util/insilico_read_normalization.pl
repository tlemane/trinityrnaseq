...
main: {
    ...
    if ($USE_KMC)
    {
      $kmer_file = &run_kmc($trinity_target_fa);
    }
    else
    {
      $kmer_file = &run_jellyfish($trinity_target_fa, $SS_lib_type);
    }
...
sub convert_kmc_to_fasta {
  my ($in_kmc_db_txt, $out_kmc_db_fa, @others) = @_;

  open(FH, '<', $in_kmc_db_txt) or Error("Could not read file ($!)");
  open(NFH, '>', $out_kmc_db_fa) or Error("Cannot write to file ($!)");
...
sub run_kmc {
    ...
    my $kmc_kmer_fa_file = "kmc.K${KMER_SIZE}.min${MIN_KMER_COV_CONST}.kmers.fa";

    my $kmc_checkpoint = "$kmc_kmer_fa_file.success";

    unless (-e $kmc_checkpoint) {
        ...
        if (-s $kmc_kmer_fa_file) {
            unlink($kmc_kmer_fa_file) or die "Error, cannot unlink $kmc_kmer_fa_file";
        }

        $cmd = "kmc_tools transform $kmc_db histogram $kmc_kmer_fa_file.hsito";

        &process_cmd($cmd);

        my $kmc_tmp_kmer = "kmc.kmers.norm.$MIN_KMER_COV_CONST.txt";

        $cmd = "kmc_dump $kmc_db $kmc_tmp_kmer";

        &process_cmd($cmd);

        convert_kmc_to_fasta($kmc_tmp_kmer, $kmc_kmer_fa_file);

        unlink($kmc_db);

        ## if got this far, consider jellyfish done.
        &process_cmd("touch $kmc_checkpoint");

    }


    return($kmc_kmer_fa_file);
